...
export class CategoryService implements OnModuleInit {
  ...
  async createCategory(categoryData: CreateCategoryeDto) {
    const existingCategorySlug = await this.categoryModel.findOne({
      $or: [{ name: categoryData.name }, { slug: categoryData.slug }],
    });

    if (existingCategorySlug) {
      if (existingCategorySlug.name === categoryData.name)
        throw new ConflictException('Category Name already exists');
      if (existingCategorySlug.slug === categoryData.slug)
  ...
  async updateCategory(id: string, updateCategory: UpdateCategoryDto) {
    ...
    let duplicacy = await this.categoryModel.findOne({
      $or: [{ name: updateCategory.name }, { slug: updateCategory.slug }],
    });

    if (duplicacy) {
      if (duplicacy.name == updateCategory.name)
        throw new ConflictException('Category with the same name exist');
      if (duplicacy.slug == updateCategory.slug)
        throw new ConflictException('Category with the same slug exist');
    }
  ...
  async deleteCategory(id: string) {
    ...
    const categoryByProduct = await lastValueFrom<any>(
      this.inventoryService.FindOneProductByCategory({
        categoryID: category._id,
      }),
    );
    if (categoryByProduct && categoryByProduct.productID)
